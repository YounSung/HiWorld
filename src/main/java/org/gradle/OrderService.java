...
public class OrderService {
	...
	public void assignAgent(int id, AgentService agentservice){
		...
		double dist = distFrom(list.get(0).getLat(),list.get(0).getLng(), orders.get(id).getLat(), orders.get(id).getLng());
		orders.get(id).assignAgent(list.get(0).getId());
		for(int i=1; i<list.size();i++){
			if(dist>=distFrom(list.get(i).getLat(),list.get(i).getLng(), orders.get(id).getLat(), orders.get(id).getLng())){
